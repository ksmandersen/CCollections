...
\end{lstlisting}
	...
	As for the hashing algorithm we use, it's a very simple one, and really not a very good one. The ideal solution would to include a hashing library in the project, to be sure that the algorithm is fast and has as few collisions as possible. Some libraries to consider could be CMPH - C Minimal Perfect Hashing Library (CMPH - C Minimal Perfect Hashing Library) or gperf - GNU Perfect hashing] (http://www.gnu.org/software/gperf/#TOCintroduction).
