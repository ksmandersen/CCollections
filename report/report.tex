...
The way we, in the cc_object struct, supports multiple different types of values, is via a union declaration. The cc_object structs value field is a union of all the different types it supports. A union in C works by allocating space in the struct for the largest type, and each of the types points to this allocated memory. There's type safe access to all the different types, but they share the memory, so that only one of them can be valid at a time. This keeps the cc_object struct as small as possible, while retaining type safety. Other than the value, we also have a string, which is used to identify the objects type, like gettings its class name.

\begin{lstlisting}[label=cc_object-struct,caption=Internal structure of the cc_object]
