\documentclass[table,nottoc]{ituthesis}
...
\section*{Abstract}\label{sec:preface}
...
The entire source, with tests and documentation, for the project is available for download on GitHub at \\\url{github.com/ksmandersen/CCollections}.
\clearpage
...
Data structures are a natural asset in the development of any application. Unlike many languages, C does not come with a full range of collections, neatly packaged together into a collection library. In fact there is not a single collection type in the standard library. Developers find themselves faced with two options. Either write the implementation needed from scratch or go online and find an existing implementation that looks usable.
...
In examining the existing collection libraries available for C, it is found that there is room for improvement.  This project will aim to describe and implement a dedicated collection library for C that fills some of the gap between existing solutions for C and the OO-languages.
...
Manual memory management is often a big hurdle for people coming from garbage collected languages. With the right techniques and code structure, it can become less of a problem, but it's still something that can cause not only headaches during development and debugging, but also commonly causes stability- and security problems when released, if not all bugs are found. Especially in collections are programmers more often tempted to just create a static array instead of something that is dynamically expanding, because of the trouble of having to keep track of when you don't need it any more. This leads to both worse code, but also buffer overflow issues. And, apart from direct code problems, garbage collection is just a convenience that many high-level programmers expects to have. This is why we want our collections to be garbage collected.

Since there is no support for garbage collection in the C runtime, it has to run on top of the standard memory allocation. There are a number of libraries to do this, the most popular of these being the Boehm-Demers-Weiser conservative garbage collector\footnote{\url{http://www.hpl.hp.com/personal/Hans_Boehm/gc/}} (also known just as Boehm's garbage collector), which is used as the garbage collector in Mono and as a leak detector in the Mozilla project. It has replacement functions for \highlight{malloc} and \highlight{realloc}, which will instead allocate memory in heap blocks allocated by the garbage collector, and associate mark-ing bits. For collecting memory, it uses mark-sweep to find unused memory blocks and free them \cite[chap. 10]{Sestoft2012}. Finalizers can also be registered for a block of memory, which could be used to replicate the functionality of deconstructors.
...
The most obvious problem we encounter is how to have an insert function, which can insert any kind of object. A collection should be able to contain every type of object, which isn't a problem in object oriented languages, where there is a system of classes and instances, but is a little more tricky in C, since there are multiple distinct types of objects. We will want a function to insert both a char, which is a single-byte stack variable, an int, a multi-byte stack variable, a string, which is a pointer to a block of memory with an unknown size, along with whatever data structure the user has made.
...
One of the goals in this project is to have consistent and well organized interfaces to our collections. We have made a style guide, that all code should follow, that is inspired both by classic C-style naming and more Java-style method naming.
	...
	\subsection{Style guide for public interfaces}\label{sec:arch_style_public}
		...
		\begin{description}
			...
			\item Everything defined in a public interface should have documentation.
		\end{description}
...
As noted earlier only some of the libraries encountered in our search on the internet had testing implemented. Some had a few tests per collection type to make sure that the collection sort-of worked. A few had full-blown test suites with all public functionality tested. A lot of had no tests or no tests checked in to the public source at least.

The library for this project will be using unit tests with coverage of all collections and all public API functions. This will serve three main goals of the library.

\begin{enumerate}
\item The library is well-tested. Every time a change is made to a collection or another part of the library, which may affect several collections, all the tests can be run to check nothing breaks. This dramatically lowers the chance of small bugs creeping in.
\item Tests drive implementation forward. This is also known as Test-Driven Development; TDD for short. Once the general structure of the library has been determined features can be fleshed out by writing the tests first and then the implementation. This will both ensure all API is tested but also that there is no API features that has not been tested.
...
Computer Science is a field in constant movement. A lot of that movement can be credited to the movement of open source software. Developers sharing their code with the public drives the field forward. Letting other developers learn from their code, enabling them to contribute and improve existing software as well os innovate. It is in this spirit this library will released as open source software. Although it would seem that this topic is more philosophical than architectural it does have benefits other than attribution and openness. Releasing the library as open source software will allow users to peek inside the depths of the library. This brings a sense of transparency to the user of the library. It also allows the community to give back to other users by extending and improve the library.

As mention in \autoref{sec:preface} the library will be made publicly available on \\\href{https://github.com/ksmandersen/CCollections}{github.com/ksmandersen/CCollections}. The library will be licensed under the BSD-2 license\footnote{More information on \url{http://www.tldrlegal.com/license/gnu-general-public-license-v3-(gpl-3)}}, making the software free to modify, distribute and use commercially.
...
\begin{itemize}
	\item Using return values from functions (-1, -2, etc. corresponding to an error)
	\item With out parameters. Functions have a last parameter that will contain an error message or code that must be checked by the calling code.
	\item With callbacks. Functions have a callback parameter that takes a function pointer to a function that will be called with a error message or code.
	\item With global callbacks. Like callbacks but there is only a global function that will be called no matter where the error occurs.
	\item Using an error state. Each instance of a collection holds a state which indicates whether an error has occurred along with an error code or an error message. Developers will have to check to check the error state for each call.
	\item Using a long jump. This can be used to create something like the try..catch pattern.
\end{itemize}

The optimal solution for a project like this would be the last option. With a setjmp/longjmp wrapper and an exception object, we could make something that would very much resemble the try..catch pattern, which is known from newer languages. This approach allows both checking for errors after a single function call, jumping out of a block of code on error, and having a global handler for uncaught exceptions. Multiple libraries for replicating this functionality exists. One of the more popular choices is exceptions4c\footnote{\url{https://code.google.com/p/exceptions4c/}}.
...
The way we, in the cc\_object struct, supports multiple different types of values, is via a union declaration. The cc\_object structs value field is a union of all the different types it supports. A union in C works by allocating space in the struct for the largest type, and each of the types points to this allocated memory. There's type safe access to all the different types, but they share the memory, so that only one of them can be valid at a time. This keeps the cc\_object struct as small as possible, while retaining type safety. Other than the value, we also have a string, which is used to identify the objects type, like gettings its class name.

\begin{lstlisting}[label=cc_object-struct,caption=Internal structure of the cc\_object]
...
As a side effect of great architectured extensibility of the library it becomes very easy and convenient to reuse code across the library. All collections of course share the cc\_objects and enumerators code. But a collection can be built as extension of other collections.
...
\end{lstlisting}
	...
	\begin{lstlisting}[label=cc_enumerator-impl-example,caption=Implementation of linked list's get\_enumerator]	
...
\section{Testing}
	...
	As noted in \autoref{sec:arch_testing} this project will be covering its implementation with functional unit tests. For the implementation of the unit tests a framework called Unity\footnote{\url{https://github.com/ThrowTheSwitch/Unity}} has ben chosen. It was picked from a list of 45 competing libraries \footnote{\url{http://en.wikipedia.org/wiki/List\_of\_unit\_testing\_frameworks\#C}}. It features xUnit\footnote{\url{http://en.wikipedia.org/wiki/XUnit}} compliant unit tests and is very light weight. This means that it can easily be bundled with library src and does not have to be installed separately from the source like some of the other choices.

	The library features a total of 147 unit tests across all public functions for both the data structures and the cc\_objects and enumerators. The tests can easily be compiled and run straight from the command line using the Makefile. When the command \highlight{make test} is run it will run all unit tests for library and tell how many tests failed or passed.
...
\chapter{Conclusion}
	...
	\section{Discussion}\label{sec:discussion}
		...
		\subsection{Outstanding functionality}
			...
			Lastly, in \autoref{sec:arch_error_handling} we had a suggestion for how to build a system for handling errors. Unfotunately, we didn't get to implement this, which means that error handling is lacking in our library, as of its current state. For future work on the project, this will be a high priority task, both the design of an exception flow architecture, as well as the implementation of this.
